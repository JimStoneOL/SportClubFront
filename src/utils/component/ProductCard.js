...
const formOP={
  ...
  count:1
    ...
    try {
      const fetched = await request('http://localhost:8080/api/op/create', 'POST', {...formOP}, {
        Authorization: `Bearer ${token}`
      })
    } catch (e) {}
  }
  ...
  const getIssue = useCallback(async () => {
    ...
    <div class="rounded overflow-hidden shadow-lg">
      ...
      <div class="px-6 py-4">
        ...
        <button onClick={orderHandler} className="text-white bg-black rounded-full w-32 ">Оформить</button>
      </div>
    </div>
